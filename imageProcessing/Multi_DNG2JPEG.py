...
for i in range(startNo, endNo+1):
    img = f'.\imageProcessing\imageFile\DJI_0{i}.DNG'
    rawpy.imread(img)
    with rawpy.imread(img) as raw:     # raw데이터를 읽는다.
        bgr = raw.postprocess()        # 들여쓰기 할 것
    rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
    cv2.imwrite(f'.\imageProcessing\imageFile\postProcessImage\DJI_0{i}.jpg', rgb, [cv2.IMWRITE_JPEG_QUALITY,compressure])
